...
Using qplot to show a bar plot, `fill` to color the bars. Check the number of records per Species
...
qplot automatically decides which plot to use based on supplied variables, looks like. 
...
Maybe I can drop the `x=` and `y=` specifications too for the `aes()` method?
...
ggplot(df,
  ...
  coord_flip()
```
Some data points are clearly outside lines extending outwards the box but I cannot conclude that these are erroneous data. Since I cannot gather more information about these "outlier" measurements for this dataset, I will assume they are correct data for now. 
This is also true for the rest of the box plots.
